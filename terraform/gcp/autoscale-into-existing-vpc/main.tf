...
resource "google_compute_instance_template" "instance_template" {
  name = substr(format("${var.prefix}-tmplt-%s", replace(uuid(), "-", "")), 0, length("${var.prefix}-tmplt-")+5)
...
resource "google_compute_region_instance_group_manager" "instance_group_manager" {
  region = var.region
  name = substr(format("${var.prefix}-igm-%s", replace(uuid(), "-", "")), 0, length("${var.prefix}-igm-")+5)
  version {
    instance_template = google_compute_instance_template.instance_template.id
    name = "${var.prefix}-tmplt"
  }
  base_instance_name = substr(format("${var.prefix}-%s", replace(uuid(), "-", "")), 0, length("${var.prefix}-")+5)
}
resource "google_compute_region_autoscaler" "autoscaler" {
  region = var.region
  name = substr(format("${var.prefix}-autoscaler-%s", replace(uuid(), "-", "")), 0, length("${var.prefix}-autoscaler-")+5)
