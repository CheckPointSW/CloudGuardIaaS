...
output "tap-security_group" {
  value = aws_security_group.tap_sg.id
}
...
output "tap-tap_lambda_description" {
  value = "The 'tap' lambda creates traffic-mirror sessions for all mirrorable and non-blacklisted instances in the TAP gateway's vpc. This lambda is invoked during TAP deployment, scheduled events and upon an ec2 instance in Running state"
}
...
output "tap-termination_lambda_description" {
  value = "The 'Termination' lambda deletes traffic-mirror sessions for all instances in the TAP gateway's vpc. Should be invoked manually before destroying the environment."
}
