...
def get_mirror_session_ids():
    response_mirror_sessions = ec2_client.describe_traffic_mirror_sessions(
        Filters=[{'Name': 'traffic-mirror-target-id',
                  'Values': [tmt_id]}])
...
def lambda_handler(event, context):
    ...
    print('Deleting traffic mirror filter')
    ec2_client.delete_traffic_mirror_filter(TrafficMirrorFilterId=tmf_id)
    print('Deleting traffic mirror target for TAP gateway')
    ec2_client.delete_traffic_mirror_target(TrafficMirrorTargetId=tmt_id)
